...
class Point2D: # implements Comparable<Point2D> {
    ...
    def theta(self): return Math.atan2(self._y, self._x)
    ...
    def _angleTo(self, that):
        double dx = that.x - self._x;
        double dy = that.y - self._y;
    ...
    private static class _YOrder implements Comparator<Point2D> {
        def compare(p, q):
    ...
    private static class _ROrder implements Comparator<Point2D> {
        def compare(p, q):
    ...
    private class _Atan2Order implements Comparator<Point2D> {
        def compare(q1, q2):
            angle1 = _angleTo(q1)
            angle2 = _angleTo(q2)
    ...
    private class _PolarOrder implements Comparator<Point2D> {
        def compare(q1, q2):
            ...
            else return -ccw(Point2D.self, q1, q2);     # both above or below
    ...
    def equals(self, other):
        ...
        that = other
        return self._x == that.x && self._y == that.y
